...
class BlockTemplate(halfnode.CBlock):
    ...
    def __init__(self, coinbasers_value, job_id):
        super(BlockTemplate, self).__init__()
        
        self.job_id = job_id 
        self.coinbasers_value = coinbasers_value
    ...
    def fill_from_rpc(self, data):
        ...
        coinbase = self.coinbase_transaction_class(self.coinbasers_value, data['coinbasevalue'], data['coinbaseaux']['flags'], data['height'], settings.COINBASE_EXTRAS)
        ...
        self.timedelta = self.curtime - int(time.time()) 
        self.merkletree = mt
