...
// console.log(helper.now(), '执行一分钟任务');
// });
...
function check() {
    async.parallel({
        ...
        users : function(callback) {// 开放了通知功能的矿池用户
            pool_db.query('select user_id,username,notify_type,notify_last_time from pool_account where is_notify_open = ?', [ 1 ], function(err, poolAccounts) {
                if (err) {
                    callback(null, []);
                }
                else {
                            ...
                            _.each(userList, function(u) {
                                ...
                                if (notify_type) {
                                    if (notify_type.indexOf('0') !== -1 && u['tel_active'] == 1 && u['tel']) {
                                        var tel = '13693320901';// u['tel'];
                                        if(username.length>12){
                                            username = username.substring(0,12)+'..';
                                        }
                                        sms.sendSMS(tel, '尊敬的ASICME矿池用户'+username+'，您的矿机已离线超过30分钟，按照您的设置特此提醒【asicme.com】');//52个字符加用户名
                                    }
                                    if (notify_type.indexOf('1') !== -1 && u['email_active'] == 1 && u['email']) {
                                        var email = 'echover@qq.com';// u['email'];
                                        mail.sendMail(email, '来自ASICME矿池的提醒', '尊敬的ASICME矿池用户【' + username + '】您好，您的矿机已经超过30分钟未连接到矿池，按照您的通知设置，特此邮件提醒!【asicme.com】');
                                    }
                                }
                            });
                        }
